...
resource "tencentcloud_cfw_edge_firewall_switch" "switches" {
  count = length(var.switches)

  enable      = var.switches[count.index].switch_enable
  switch_mode = var.switches[count.index].switch_mode
  public_ip   = var.switches[count.index].switch_public_addr
  subnet_id   = var.switches[count.index].switch_subnet_id

  depends_on = [ tencentcloud_cfw_sync_asset.sync_asset ]
}
